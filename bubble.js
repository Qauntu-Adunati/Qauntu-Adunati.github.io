...
function Point(x, y, z, size, color) {
    ...
    // move to vertex C
    ctx.lineTo(this.curPos.x + dx + this.radius * 1.3, this.curPos.y + dy + this.radius * 1.3);
    // fill our shape
    ctx.fill();
...
ctx.beginPath();
            ctx.arc(this.curPos.x + dx, this.curPos.y + dy, this.radius/2, 0, Math.PI *2,true)

            ctx.fill();
             ctx.arc(this.curPos.x + dx, this.curPos.y + dy,  (Math.random() * 10), 0, Math.PI * 2, true);
             //this.curPos.z = Math.random() * 4;
             var z = Math.floor(Math.random()* 20);
             if(this.curPos.z < z){
                 this.curPos.z ++;
             }else if(this.curPos.z > z){
                 this.curPos.z ++;
             }
             if(this.curPos.x <dx){
                 this.curPos.x++;
             }else  if(this.curPos.x > dx){
                 this.curPos.x--;
             }

             if(this.curPos.y <dy){
                 this.curPos.y++;
             }else  if(this.curPos.y > dy){
                 this.curPos.y--;
             }
